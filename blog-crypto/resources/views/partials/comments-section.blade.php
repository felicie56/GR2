        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-400">
                    Chia sẻ góc nhìn của bạn về nội dung này. Vui lòng trao đổi lịch sự và tránh spam hoặc nội dung lừa đảo.
                </p>
            </div>

            <div class="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-300">
                Tổng:
                <span id="comments-total-count" class="font-bold text-white">{{ $commentCount }}</span>
                bình luận
            </div>
        </div>
    </div>
            ...
            <form method="POST"
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <label for="comment-content" class="block text-sm font-bold text-slate-200 mb-2">
                            Viết bình luận
                        </label>
                        ...
                        <div class="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <p class="text-xs text-slate-500">
                                Bình luận có thể được admin kiểm tra và xóa nếu vi phạm quy định cộng đồng.
                            </p>
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-slate-300">
                            Bạn cần đăng nhập để tham gia thảo luận, đặt câu hỏi hoặc chia sẻ quan điểm về bài viết.
                        </p>
                    </div>
        ...
        @if ($commentCount === 0)
            <div id="comments-empty-state" class="p-8 md:p-10 text-center">
                ...
                <p class="mt-2 text-slate-400">
                    Hãy là người đầu tiên chia sẻ góc nhìn về nội dung này.
                </p>
            </div>
        @endif
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <div class="mt-1 text-xs text-slate-500">
                                        {{ $comment->created_at ? $comment->created_at->format('d/m/Y H:i') : 'Không rõ thời gian' }}
                                    </div>
                                </div>

                                @auth
                                    @php
                                        $canDeleteOwnComment = $comment->user_id === auth()->id();
                                        $hasAdminRole = method_exists(auth()->user(), 'hasRole') && auth()->user()->hasRole('ADMIN');
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const totalCount = document.getElementById('comments-total-count');
        const blogCommentStat = document.getElementById('blog-comment-count-stat');
        ...
        function renderComment(comment) {
            const userName = comment.user_name || 'Người dùng';
            const avatar = userName.charAt(0).toUpperCase();
            const createdAt = comment.created_at || 'Vừa xong';
                ...
                <article class="p-6 md:p-8 hover:bg-white/[0.025] transition">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-4 rounded-3xl border border-white/10 bg-slate-950/60 p-5">
                                <p class="text-left text-slate-200 whitespace-pre-wrap break-words [overflow-wrap:anywhere] leading-7">${escapeHtml(content)}</p>
                            </div>
                        </div>
                    </div>
                </article>
        ...
        form.addEventListener('submit', async function (event) {
            ...
            if (!content) {
                if (errorBox) {
                    errorBox.textContent = 'Vui lòng nhập nội dung bình luận.';
            ...
            /*
             * Quan trọng:
             * Phải tạo FormData trước khi set loading.
             * Không disable textarea, vì field disabled sẽ không được gửi trong FormData.
             */
            ...
            try {
                ...
                if (!response.ok) {
                    let message = data.message || 'Không thể gửi bình luận. Vui lòng thử lại.';

                    if (data.errors && data.errors.content && data.errors.content.length > 0) {
                ...
                if (totalCount && typeof data.comment_count !== 'undefined') {
                    totalCount.textContent = data.comment_count;
                }

                if (blogCommentStat && typeof data.comment_count !== 'undefined') {
                    blogCommentStat.textContent = data.comment_count + ' bình luận';
                }

                textarea.value = '';
            } catch (error) {
                if (errorBox) {
                    errorBox.textContent = error.message || 'Không thể gửi bình luận. Vui lòng thử lại.';
                    errorBox.classList.remove('hidden');
                }
            } finally {
                setLoading(false);
