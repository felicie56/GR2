<x-guest-layout>
    ...
    <style>
        .admin-comment-moderation .admin-comment-copy,
        .admin-comment-moderation .admin-comment-meta {
    ...
    <div class="admin-comment-moderation max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                    <div>
                        ...
                        <p class="mt-4 max-w-2xl text-slate-300 leading-relaxed">
                            Bình luận mới sẽ ở trạng thái chờ duyệt. Chỉ bình luận được admin duyệt mới xuất hiện trên Blog hoặc Tin tức.
                        </p>
                    </div>
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="{{ route('admin.comments.index', ['status' => 'all', 'type' => $type, 'search' => $search]) }}"
               class="rounded-3xl border border-white/10 bg-white/[0.04] p-5 hover:bg-white/[0.07] transition">
                <div class="text-sm text-slate-400">Tất cả</div>
                <div class="mt-2 text-3xl font-black text-white">{{ number_format($totalComments ?? 0) }}</div>
            </a>

            <a href="{{ route('admin.comments.index', ['status' => 'pending', 'type' => $type, 'search' => $search]) }}"
               class="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-5 hover:bg-amber-400/15 transition">
                <div class="text-sm text-amber-200">Chờ duyệt</div>
                <div class="mt-2 text-3xl font-black text-white">{{ number_format($pendingComments ?? 0) }}</div>
            </a>

            <a href="{{ route('admin.comments.index', ['status' => 'approved', 'type' => $type, 'search' => $search]) }}"
               class="rounded-3xl border border-emerald-400/20 bg-emerald-400/10 p-5 hover:bg-emerald-400/15 transition">
                <div class="text-sm text-emerald-200">Đã duyệt</div>
                <div class="mt-2 text-3xl font-black text-white">{{ number_format($approvedComments ?? 0) }}</div>
            </a>

            <a href="{{ route('admin.comments.index', ['status' => 'rejected', 'type' => $type, 'search' => $search]) }}"
               class="rounded-3xl border border-rose-400/20 bg-rose-400/10 p-5 hover:bg-rose-400/15 transition">
                <div class="text-sm text-rose-200">Từ chối</div>
                <div class="mt-2 text-3xl font-black text-white">{{ number_format($rejectedComments ?? 0) }}</div>
            </a>
        </section>

        <section class="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 md:p-6">
            <form method="GET" action="{{ route('admin.comments.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div class="md:col-span-5">
                    <label for="search" class="block text-sm font-semibold text-slate-300 mb-2">Tìm kiếm</label>
                    <input id="search"
                           name="search"
                           value="{{ $search }}"
                ...
                <div class="md:col-span-3">
                    <label for="status" class="block text-sm font-semibold text-slate-300 mb-2">Trạng thái</label>
                    <select id="status"
                            name="status"
                            class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-slate-100 focus:border-blue-400 focus:ring-blue-400/30">
                ...
                <div class="md:col-span-2">
                    <label for="type" class="block text-sm font-semibold text-slate-300 mb-2">Loại</label>
                    <select id="type"
                            name="type"
                            class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-slate-100 focus:border-blue-400 focus:ring-blue-400/30">
        ...
        <section class="rounded-[2rem] border border-white/10 bg-white/[0.04] overflow-hidden shadow-2xl shadow-slate-950/20">
            <div class="p-5 md:p-6 border-b border-white/10">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h2 class="text-2xl font-black text-white">Danh sách bình luận</h2>
                        <p class="mt-1 text-sm text-slate-400">Kiểm tra nội dung trước khi cho phép hiển thị công khai.</p>
                    </div>

                    <div class="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-slate-300">
                        Kết quả: <span class="font-bold text-white">{{ number_format($comments->total()) }}</span>
                    </div>
                </div>
            </div>
            ...
            @if ($comments->count() === 0)
                <div class="p-10 text-center">
                    ...
                    <h3 class="mt-5 text-xl font-bold text-white">Không có bình luận phù hợp</h3>
                    <p class="mt-2 text-slate-400">Hãy thử thay đổi trạng thái, loại hoặc từ khóa tìm kiếm.</p>
                </div>
            @else
                <div class="divide-y divide-white/10">
                    @foreach ($comments as $comment)
                        @php
                            ...
                            $commentType = $comment->blog_post_id ? 'blog' : 'news';
                            $commentTypeBadge = $typeConfig[$commentType];
                            $relatedTitle = $comment->blogPost?->title ?? $comment->news?->title;
                            $relatedUrl = null;

                            if ($comment->blogPost?->slug && strtolower((string) $comment->blogPost->status) === 'approved') {
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                <div class="lg:col-span-8 min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        <span class="rounded-full border border-white/10 bg-slate-950/50 px-3 py-1 text-xs text-slate-400">
                                            {{ $comment->created_at?->format('d/m/Y H:i') ?? 'Chưa rõ thời gian' }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="mt-4 flex items-start gap-3">
                                        ...
                                        <div class="admin-comment-meta min-w-0">
                                    ...
                                    <div class="mt-5 w-full rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-4">
                                        <p class="admin-comment-copy text-left text-slate-200 whitespace-pre-wrap break-words [overflow-wrap:anywhere] leading-7">
                                            {{ $comment->content }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="lg:col-span-4">
                                    <div class="rounded-3xl border border-white/10 bg-slate-950/50 p-5 space-y-5">
                                        <div>
                                            <div class="text-xs uppercase tracking-wide text-slate-500">Nội dung liên quan</div>
                                            <div class="mt-2 text-sm font-semibold text-white break-words [overflow-wrap:anywhere]">
                                                {{ $relatedTitle ?? 'Nội dung không còn tồn tại' }}
                                            </div>
                                            @if ($relatedUrl)
                                                <a href="{{ $relatedUrl }}"
                                        ...
                                        @if ($comment->reviewed_at)
                                            <div class="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">
                                                Xử lý bởi <span class="font-semibold text-slate-200">{{ $comment->reviewer?->name ?? 'Admin' }}</span><br>
                                                lúc {{ $comment->reviewed_at->format('d/m/Y H:i') }}
                                            </div>
                                        @endif
